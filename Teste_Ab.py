...
Original file is located at
    https://colab.research.google.com/drive/14XIT-kpgeM1g0KvXPYtgQtFZxIcTnKxT
"""
...
def energy_function(pb_positions, iot_positions, E_min, E_max, PT, N, frequency, alpha, kappa, tau):
    ...
    for m in range(M):
      for k in range(K):
          for n in range(N):
            psi_mj[m][k][n] = np.sqrt(PT / N) * (H_los[m][k][n]/np.abs(H_los[m][k][n])) # <----- ALTERAR ENTRE H E HLOS AQUI
      ...
      for k in range(K):
        for j in range(K):
          P_k_array[k][j] = received_power(betas[m][j], psi_mj[m][k], H_los[m][j]) # <------ ALTERAR ENTRE H E HLOS AQUII
